...
- Add durable inbound idempotency and restart-safe task admission, explicit
  task/lane controls, role-based access, rapid-message coalescing, scheduler
  metrics, safe merged-worktree retention, and cross-lane file-overlap alerts.
...
- Persistent `/park`, plus `/wake`, `/diag`, and `/replay` topic lifecycle and
  delivery tools; intentional parking remains distinguishable from a crash
...
- Provider-aware `directory-provider-number` names across Claude, Codex,
  ...
  `/autoname` migration with deterministic slot reservation for legacy topics.

### Changed

- Live View now records its per-view network circuit breaker explicitly; a
