...
- Task runtime, lease activity, and provider progress are separate clocks. Slow/stuck watchdog transitions must not refresh provider progress. `dispatch.json` persists the accepted user-turn transcript offset so hookless restart discovery can replay terminal events idempotently instead of starting at EOF and leaking a scheduler slot.
...
- `window_tick/__init__.py` — `tick_window(runtime=None, adaptive=True)` (thin orchestrator; accepts optional `PollingRuntime`). Adaptive backoff: idle windows (no pane-content change per `WindowPollState.last_change_ts`, no transcript activity for `IDLE_BACKOFF_AFTER`) skip the tick — pane capture included — all but every `IDLE_TICK_EVERY`th cycle; never skips dead windows, pending queues, interactive UIs, or RC debounce. Toggle `CCGRAM_ADAPTIVE_POLL` (default on) read in `status_poll_loop`.
...
- `topic_creation_draft.py` — typed accessor (`TopicCreationDraft`) for the 15 `context.user_data` keys used across the topic-creation flow; `_browser_flow_stale`, `_required_selected_path`, `mark/clear_launch_in_progress` + `_launch_in_progress` (window-launch in-flight guard) module helpers. Single source of truth for all flow-state key names.
...
- `provider_mode_callbacks.py` — CB_PROV_SELECT / CB_MODE_SELECT callbacks; `_validate_provider_select` double-click guard; calls `launch_window` after provider/mode resolved. Launch is wrapped in `mark/clear_launch_in_progress` (PENDING_LAUNCH_THREAD): a duplicate tap during the multi-second launch answers "⏳ creating…" instead of misreporting "Selection expired"; the picker is also edited to "⏳ creating…" before the launch await.
