...
- **已完成的话题**（`--autoclose-done`，默认：30 分钟）—— 当 CLI 提供方明确报告任务完成状态时，话题在 30 分钟后自动关闭。
- **已死亡的会话**（`--autoclose-dead`，默认：10 分钟）—— 当 CLI 进程崩溃或 tmux/herdr 窗口被外部终止时，话题在 10 分钟后自动关闭。

普通空闲不等于会话结束：只要 CLI 窗口仍存活且没有明确的完成/死亡终态，话题会继续保留。自动关闭采用 Telegram 的“关闭话题”，聊天记录不会删除；重新打开后仍可继续使用。
...
- `/task_retry [T编号]` 只重新发送提交键，不重新发送问题正文；也可使用异常回执
  上的“重试提交”或“取消任务”按钮。已确认启动但超过
  `CCGRAM_TASK_PROGRESS_WARN_SECONDS` 没有会话增量时，状态进入“🟠 可能停滞”，而不是
  模糊的“长时间无新进展”。总览分别显示“已运行”和“最近进展”，前者是任务总耗时，
  后者才是没有可见输出的时长。疑似停滞回执提供“检查状态、继续等待、取消任务”：
  “继续等待”只延长安全租约，不会伪造 CLI 新进展；“检查状态”会核对真实 CLI 窗口。
- `dispatch.json` 同时保存本轮用户消息在 Provider 转录中的字节位置。服务重启时，若
  `monitor_state.json` 尚未来得及跟踪该会话，ccgram 会从这个精确位置恢复读取；旧版本
  记录没有位置时，按任务开始时间在最近 16 MiB JSONL 中寻找安全边界。重复过程消息由
  Delivery ID/Outbox 幂等抑制；发现 `task_complete` 或其他 Provider 最终事件后自动把
  Inbound、回执、总览和并发槽一起收敛为完成，不需要成员再发送一条消息唤醒。
...
任务状态采用明确的四级语义：`🟢 正常执行` 表示近期有真实 Provider 进展；
`🟠 可能停滞` 表示 CLI 已确认启动但超过阈值没有新转录，既不宣称完成，也不直接判定
卡死；`🔴 已确认异常` 表示窗口不存在或提交未确认，通道保持关闭失败；`✅ 已完成`
表示最终事件已被识别并进入幂等投递。总览中的“已运行 45 分钟”和“最近进展 5 分钟前”
是两个独立时间，避免把任务总时长误读成无进展时长。
