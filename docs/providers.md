...
在已经绑定 CLI 的 Telegram 话题中，可以查询或持久切换审批模式：

```text
/approval          # 查看当前模式和提供方
/approval yolo     # 切换到 YOLO
/approval normal   # 切换回标准审批模式
```

也支持中文参数：`/approval 开启`、`/approval 关闭`、`/approval 标准`。YOLO/bypass 只能由管理员启用。切换会先启动并验证替代 CLI，成功后才替换旧窗口；失败时保留原窗口和原模式。为了避免中断写文件或工具调用，有活动任务时会拒绝切换。切换会保留旧转录，并把最近上下文交接给新会话。该模式写入窗口状态，因此服务重启、CLI 退出后自动拉起、恢复及继续流程都会继续使用所选模式。

Pi 和 Shell 没有对应的宽松启动模式，命令会明确提示不支持，不会修改状态。
...
Codex 的交互式提示（问题列表、权限提示及其他选择 UI）通过 pyte 从终端屏幕内容中检测，并以内联键盘控件呈现。这套处理位于公共交互层，因此 Claude、Codex、Gemini、Pi 等 Provider 只要输出可识别的终端单选 UI，都会得到相同能力。

明确且连续的 `1.`–`8.` 单选项会直接显示为 Telegram 按钮；只有两个明确选项的 Yes/No 提示会显示“是/否”。多选框、非连续编号、多个等距菜单或其他有歧义的内容不会被猜测，仍使用方向键和 Enter。按钮与 Telegram 消息、话题、CLI 窗口及提示代次绑定；提示更新、已选择、双击或服务恢复后的旧按钮都会显示“提示已过期”，不会把答案发到新问题或其他成员的窗口。
