...
| `CCGRAM_TYPING`                                      | `1`                            | Telegram "typing…" indicator: refreshed only while the agent is genuinely producing output (transcript activity within 60s), so a long think/spinner phase stops showing a misleading perpetual "typing"; set `0` to suppress it entirely (the 🟢 topic emoji + status bubble still convey a busy agent) |
...
| `CCGRAM_AUTOCLOSE_ACTION`                            | `close`                        | How an expired topic is retired: `close` archives it and **keeps the whole history** (an archive notice is posted first); `delete` is the legacy behaviour — Telegram destroys every message in the topic along with it, so it is opt-in |
| `CCGRAM_DESTRUCTIVE_ALERTS`                          | `1`                            | Operator DM for irreversible actions (topic retired, window killed): sent **only for unattended, automatic** ones, immediately and without burst aggregation (the failure mode here is a single silent event). User-confirmed destruction is audited but never DM'd. Set `0` to stop the DMs; the audit log and the `ccgram_destructive_actions` metric keep running |
| `CCGRAM_MASS_DEATH_THRESHOLD`                        | `3`                            | Mass-death circuit breaker: this many window deaths inside the detection window means an infrastructure event (e.g. a tmux server restart), not that many user intentions, so all **automatic** destructive cleanup stands down. Set `0` to disable the breaker |
| `CCGRAM_MASS_DEATH_WINDOW`                           | `120`                          | Detection window in seconds — how close together deaths must fall to count as a burst |
| `CCGRAM_MASS_DEATH_SUSPEND`                          | `30`                           | How long (minutes) automatic cleanup stays suspended after a trip. **Deliberately much longer than the detection window**: in the 2026-07-25 incident the destruction happened ~17 minutes after the deaths, so a breaker that reset in minutes would have been useless. Nothing is lost while suspended — timers stay armed and re-evaluate afterwards |
| `CCGRAM_DESTRUCTIVE_DRYRUN`                          | _(off)_                        | Rehearsal mode: unattended destructive paths decide as usual and record what they *would* have done (audit log + `outcome=skipped_dryrun` metric), but never execute. Use it to confirm the cleanup policy matches your intent before trusting it with real topics and processes. User-confirmed destruction is unaffected |
