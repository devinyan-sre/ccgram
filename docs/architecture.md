...
| Transactional approval-mode replacement           | `/approval` passes an explicit `normal`/`yolo` override through `provider_handoff`; the replacement CLI is verified before rebinding, active tasks block the operation, failure leaves the original window untouched, and success persists the mode and updates the topic badge                                                                                                                                                                        |
