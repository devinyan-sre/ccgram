...
- **媒体任务不串线** — 群内图片/文件与文字共用 @唤醒、调度和原消息关联；未 @ 或未回复机器人的媒体不会触发 CLI
- **可控任务取消** — 任务短编号、排队 ETA、两阶段 Ctrl+C 确认与管理员强制停止，适配所有 CLI
- **同一成员显式并行** — `/task_parallel` 为独立问题创建专属 CLI、Provider 会话和 Git worktree；非 Git 运维目录可显式启用共享 cwd 回退；回复或 `T编号` 精确补充，歧义时绝不猜测
- **提交不假死、不串题** — 每次问题和补充都等待 CLI 真实接收确认；吞回车时仅补交提交键，失败后锁住成员通道并提供中文重试/取消入口
...
- **可靠投递** — 持久化 outbox、按话题隔离队列、自动重试与 `/ops` 运行摘要
...
**Telegram 运行总览** — 新任务立即回复临时“已收到”回执；General 和当前活动话题优先原地更新，空闲话题低频巡检，不刷屏；展示成员任务、并发、排队、取消和短暂完成状态，且适配所有 Provider。[开启方法](docs/guides.md#telegram-operations-dashboard)
